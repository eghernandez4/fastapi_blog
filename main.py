...
async def user_posts_page(request: Request, user_id: int, db: Annotated[Session, Depends(get_db)]):
...
async def get_user_posts(user_id: int, db: Annotated[Session, Depends(get_db)]):
    # Can check if user exists first, or can return an empty list if user not found
    result = db.execute(select(models.Post).where(models.Post.user_id == user_id))
    posts = result.scalars().all()
    return posts
